    ...
    public class OriginalUtility {
        public string ContentOriginal(string content, string tag, string tagurl, bool inserttag, bool original, string keyword) {
            if (original) {
                ...
                for (int j = 0; j < strArray.Length; j++) {
                    string key = Regex.Match(strArray[j], "(.+)→(.+)").Groups[1].Value;
                    string str3 = Regex.Match(strArray[j], "(.+)→(.+)").Groups[2].Value;
            ...
            for (int i = 0; i < strArray2.Length; i++) {
                string str6 = Regex.Match(strArray2[i], "(.+)→(.+)→(.+)").Groups[1].Value;
                string str7 = Regex.Match(strArray2[i], "(.+)→((.+)→(.+))").Groups[2].Value;
            ...
            foreach (string str8 in dictionary2.Keys) {
                ...
                string str9 = Regex.Match(dictionary2[str8], "(.+)→(.+)").Groups[1].Value;
                string str10 = Regex.Match(dictionary2[str8], "(.+)→(.+)").Groups[2].Value;
                //if (parm.ubb) {
        ...
        public string TitleOriginal(string content) {
            ...
            for (int i = 0; i < strArray.Length; i++) {
                string key = Regex.Match(strArray[i], "(.+)→(.+)").Groups[1].Value;
                string str3 = Regex.Match(strArray[i], "(.+)→(.+)").Groups[2].Value;
                if (!dictionary.ContainsKey(key)) {
                    dictionary.Add(key, str3);
                }
            }
